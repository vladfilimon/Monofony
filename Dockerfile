...
FROM node:${NODE_VERSION}-alpine${ALPINE_VERSION} AS mono_node
...
FROM base AS mono_php_prod

#COPY --from=mono_node /srv/sylius/public/build public/build

FROM nginx:${NGINX_VERSION}-alpine AS mono_nginx
...
#COPY --from=mono_node /srv/sylius/public public/

FROM mono_php_prod AS mono_php_dev
...
FROM mono_php_prod AS mono_cron
...
FROM mono_php_prod AS mono_migrations_prod
...
FROM mono_php_dev AS mono_migrations_dev
